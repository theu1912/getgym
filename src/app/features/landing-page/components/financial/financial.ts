...
@Component({
  ...
  styles: [`
    ...
    .lock-input { width: 100%; padding: 12px; margin-bottom: 15px; border: 1px solid #ccc; border-radius: 4px; text-align: center; letter-spacing: 5px; font-size: 1.2em; box-sizing: border-box; }
    ...
    .unit-selector { display: flex; gap: 10px; margin-bottom: 20px; flex-wrap: wrap; }
    .unit-btn { background: #f1f5f9; border: 1px solid #e2e8f0; padding: 8px 16px; border-radius: 6px; cursor: pointer; font-size: 0.9em; }
    .unit-btn.active { background: #2d3436; color: white; border-color: #2d3436; }
  `],
  template: `
    <div class="dashboard-container">
      ...
      <div *ngIf="isUnlocked">
        ...
        <div class="unit-selector">
          <button class="unit-btn" [class.active]="unidadeSelecionada === 'Fazendinha'" (click)="mudarUnidade('Fazendinha')">Fazendinha</button>
          <button class="unit-btn" [class.active]="unidadeSelecionada === 'Piraquara'"  (click)="mudarUnidade('Piraquara')">Piraquara</button>
          <button class="unit-btn" [class.active]="unidadeSelecionada === 'Pinhais'"    (click)="mudarUnidade('Pinhais')">Pinhais</button>
        </div>
        ...
        <div class="financial-grid">
          ...
          <div class="fin-card">
            <div class="fin-title">Receita Livre</div>
        ...
        <div style="display: grid; grid-template-columns: 2fr 1fr; gap: 20px;">
            ...
            <table style="width: 100%; border-collapse: collapse; text-align: left;">
              ...
              <tbody>
                ...
                <tr *ngIf="despesas.length === 0">
                  <td colspan="5" style="text-align: center; padding: 30px; color: #94a3b8;">Nenhuma despesa cadastrada.</td>
                </tr>
              </tbody>
            </table>
          </div>
          ...
          <div style="background: white; padding: 25px; border-radius: 8px; box-shadow: 0 4px 15px rgba(0,0,0,0.03);">
            ...
            <div *ngFor="let plano of planos" style="margin-bottom: 16px;">
              <div style="display: flex; justify-content: space-between; margin-bottom: 4px; font-size: 0.9em; color: #334155;">
                <span>{{ plano.nome }}</span><span style="font-weight: bold;">{{ plano.percent }}%</span>
              </div>
              <div style="background: #f1f5f9; border-radius: 4px; height: 8px;">
                <div [style.width.%]="plano.percent" style="background: #10b981; height: 100%; border-radius: 4px; transition: width 0.5s;"></div>
              </div>
            </div>
          </div>
        </div>
        ...
        <div *ngIf="modalAberto" style="position: fixed; top: 0; left: 0; width: 100%; height: 100%; background: rgba(15,23,42,0.6); display: flex; justify-content: center; align-items: center; z-index: 1000;">
          <div style="background: white; padding: 30px; border-radius: 8px; width: 380px;">
            ...
            <input style="width: 100%; padding: 12px; margin-bottom: 15px; border: 1px solid #e2e8f0; border-radius: 6px; box-sizing: border-box;" placeholder="Categoria" [(ngModel)]="novaDespesa.categoria">
            <input style="width: 100%; padding: 12px; margin-bottom: 15px; border: 1px solid #e2e8f0; border-radius: 6px; box-sizing: border-box;" type="date" [(ngModel)]="novaDespesa.vencimento">
            <input style="width: 100%; padding: 12px; margin-bottom: 15px; border: 1px solid #e2e8f0; border-radius: 6px; box-sizing: border-box;" placeholder="Valor (ex: 1200.00)" type="number" min="0" step="0.01" [(ngModel)]="novaDespesa.valor">
...
export class FinancialComponent {
  ...
  planos: { nome: string; percent: number }[] = [];

  modalAberto = false;
  novaDespesa: any = { categoria: '', vencimento: '', valor: null, status: 'Pendente' };
  ...
  carregarDadosFinanceiros() {
    this.http.get<any>(`${environment.apiUrl}/api/financeiro/dados?unidade=${this.unidadeSelecionada}`, { headers: this.headers })
      .subscribe({
        ...
        error: (err) => console.error('Erro ao carregar dados financeiros:', err)
      });
  }

  abrirModal() {
    this.novaDespesa = { categoria: '', vencimento: '', valor: null, status: 'Pendente' };
    this.modalAberto = true;
  }

  salvarDespesa() {
    if (!this.novaDespesa.categoria || !this.novaDespesa.valor || !this.novaDespesa.vencimento) return;
    const payload = { ...this.novaDespesa, unidade: this.unidadeSelecionada };
    this.http.post(`${environment.apiUrl}/api/financeiro/despesas`, payload, { headers: this.headers })
      .subscribe({
        next: () => { this.modalAberto = false; this.carregarDadosFinanceiros(); },
  ...
  mudarStatus(despesa: any) {
    ...
    this.http.put(`${environment.apiUrl}/api/financeiro/despesas/${despesa.id}`, { status: novoStatus }, { headers: this.headers })
      .subscribe({
  ...
  editarValor(despesa: any) {
    const novoValor = prompt(`Novo valor para "${despesa.categoria}":`, despesa.valor);
    if (novoValor !== null && !isNaN(parseFloat(novoValor))) {
      this.http.put(`${environment.apiUrl}/api/financeiro/despesas/${despesa.id}`, { valor: parseFloat(novoValor) }, { headers: this.headers })
        .subscribe({
          next: () => this.carregarDadosFinanceiros(),
          error: (err) => console.error('Erro ao editar valor:', err)
        });
    }
  }
